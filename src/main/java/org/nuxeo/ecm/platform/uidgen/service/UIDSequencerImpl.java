...
public class UIDSequencerImpl implements UIDSequencer {
	...
	private static final Log LOG = LogFactory.getLog(UIDSequencerImpl.class);

	private static Integer currentSeqId = 0;
	...
	private static void activatePersistenceProvider() {
		...
		try {
			thread.setContextClassLoader(PersistenceProvider.class
					.getClassLoader());
			PersistenceProviderFactory persistenceProviderFactory = Framework
					.getLocalService(PersistenceProviderFactory.class);
			persistenceProvider = persistenceProviderFactory
					.newProvider("NXUIDSequencer");
			...
			return getOrCreatePersistenceProvider().run(true,
					new RunCallback<Integer>() {
						public Integer runWith(EntityManager em) {
							synchronized (currentSeqId) {
								int next = -1;
								while (next <= currentSeqId) {
									next = getNext(em, key);
								}
								currentSeqId = next;
								return next;
	...
	public int getNext(EntityManager em, String key) {
		UIDSequenceBean seq;
		try {
			seq = (UIDSequenceBean) em
					.createNamedQuery("UIDSequence.findByKey")
